...
def run(command):
    ...
    #    if output: yield output.strip()
    #return process.poll()


# The command to be run in bash
sarndbox_command = '{} {}'.format(sarndbox_path, ' '.join(flags))

for line in run(sarndbox_command):
    print(line + 'whee')
...
while True:
    ...
    try:
        ...
        kinect_reset_process = subprocess.run("KinectUtil reset all", shell=True, timeout=2, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

        print('stdout:')
        print(kinect_reset_process.stdout)
        if 'done' in str(kinect_reset_process.stdout): kinect_reset_process.kill()
